...
Пусть даны множества B_j, называемых контейнерами, вместимость каждого из которых примем за 1, и множество L={1,...,n} предметов размеров w_i, принадлежащих интервалу (0,1), i принадлежит L. Надо найти разбиение множества L на минимальное число m подмножеств B_1, B_2,...,B_m такое, что sum(W_i)<=1 для любых i из B_j, 1<=j<=m. Решение называется оптимальным, если B минимально. Минимальное B далее обозначается OPT.

В случае нашей задачи примем высоту листа А4 за вместимость контейнера. В представленной задаче высота листов А4 одинакова, как и вместимость контейнеров. Примем поля сверху, снизу и высоту места для разреза одинаковыми и для удобства высоту листа возьмем равной высоте обычного листа А4 минус поле сверху. За размер предмета примем нормализованную высоту одного экзаменационного билета плюс поле под самим билетом (место для разреза) ((высота билета+место для разреза)/высота листа). Будем рассматривать одномерные контейнеры.
